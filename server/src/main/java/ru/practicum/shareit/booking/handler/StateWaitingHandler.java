...
public class StateWaitingHandler extends Handler {
    @Override
    public List<Booking> handleRequest(State state, BookingRepository repository, Long userId,
        ...
        if (!state.equals(State.WAITING) && next != null) {
            return next.handleRequest(state, repository, userId, owner, page, now);
        }

        if (owner) {
            return repository.findAllByItemOwnerIdAndStatusOrderByStartDesc(userId, Status.WAITING, page);
        } else {
